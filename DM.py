...
def main():
    ...
    x = 0
    # the 0->final_scale range that the draw point is scaled to
    final_scale = 1.0
    color = 'b'
    fig = plt.figure()
    ...
    ax.set_ylabel("Z")
    ax.set_zlabel("Y")
    ax.set_xlim3d(0, final_scale*1.333)
...
#    model.reply()
    ...
    for frame in camera.capture_continuous(capture, format="bgra", use_video_port=True, resize=(img_width,img_height)):
        ...
        #uart_val = 0
        t2 = datetime.now()
        #print("Uart takes ", t2 - t1, " seconds")
#        imgLeft_gr, imgRight_gr = get_grayscale_lr_images(frame, img_height, img_width)
        ...
        if is_hand_hist_created:
            ...
            if (uart_val[0] & 0x01) == 0x01:
                color = 'y'
            elif (uart_val[0] & 0x02) == 0x02:
                color = 'r'
            elif (uart_val[0] & 0x04) == 0x04:
                color = 'g'
            # for 2d plotting
            #ax.scatter(draw_point[0], 0, draw_point[1],  c=color)   
            ax.scatter(draw_point[0], draw_point[2], draw_point[1], c=color)
            plt.pause(0.005)
