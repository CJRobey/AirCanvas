...
def draw_rect(frame):
    ...
    hand_rect_one_x = np.array(
        [3 * rows / 20, 3 * rows / 20, 3 * rows / 20, 9 * rows / 20, 9 * rows / 20, 9 * rows / 20, 6 * rows / 20,
         6 * rows / 20, 6 * rows / 20], dtype=np.uint32)
...
def draw_circles(frame, traverse_point):
    if traverse_point is not None:
        point = traverse_point[len(traverse_point)-1]
...
def main():
    ...
    while capture.isOpened():
        ...
        if pressed_key & 0xFF == ord('z'):
            ...
            print(hand_hist.shape, frame.shape)
